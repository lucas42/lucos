...
- **Independent BIND patching with push-based monitoring.** Each system reports health to `schedule_tracker` (as `lucos_dns_sync` already does for its `config-sync` job), which is the natural home for the SOA-serial-consistency check ADR-0003 called for — no HTTP server required. The DNS systems are bare BIND containers with no HTTP server, and (being absent an `http_port`) are excluded from monitoring's `/_info` poll list, so the check is surfaced as a scheduled job rather than an `/_info` field. lucos_monitoring#189 was closed `not_planned` precisely because service-specific checks belong in the service, not the monitoring repo.
...
- **SOA-serial-consistency monitoring** — re-home the ADR-0003 check (lucos_monitoring#189, closed `not_planned`) as a **`schedule_tracker` scheduled job running on the secondary** (cron: `dig` both nameservers' SOA serials, compare, `curl`-push pass/fail), surfaced via monitoring's existing `fetcher_scheduled_jobs`. The DNS systems are bare BIND containers with no HTTP server and no `/_info`, so an HTTP server is deliberately *not* stood up for one check. An overdue job (frequency × 3) covers a dead secondary; the same job's error push covers an up-but-stale secondary. Tracked in lucas42/lucos#217. *(Corrected from the original "`/_info`-exposed signal" wording after lucas42 flagged the absence of an HTTP server; mechanism confirmed with the monitoring owner.)*
