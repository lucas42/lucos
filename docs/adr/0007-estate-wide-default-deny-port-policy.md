...
3. **`/etc/docker/daemon.json` IPv6 enablement on avalon, xwing, salvare.** Sysadmin to confirm whether IPv6 is already enabled before raising tickets — lucas42's recollection is that it may be. If already in place, this step is no-op.
