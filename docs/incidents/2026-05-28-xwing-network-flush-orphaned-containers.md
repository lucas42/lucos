...
|---|---|---|
| The original IPv6 daemon-config fix in #192 (xwing's `fixed-cidr-v6` set to a ULA prefix) — the primary scope of #192. The live-restore-specific recovery procedure that surfaced during this incident's recovery is tracked separately as `lucas42/lucos_claude_config#98` below | lucas42/lucos#192 | Closed (completed) at 18:57:09Z |
| Sysadmin verification template: `docker network ls` returning empty is **never** an expected steady state. Rule shipped in [lucas42/lucos_claude_config@a48fe4b](https://github.com/lucas42/lucos_claude_config/commit/a48fe4b7b9c954b0211ba2e8835d8bc38a1308a7) — Quality Control checklist point 4 now requires `curl https://<service>/_info` from outside the host for each affected HTTP service after any daemon/network change, and explicitly forbids dismissing empty `docker network ls`. Extended by `lucas42/lucos_claude_config#99` (iptables `DOCKER` chain check + CI-redeploy recovery note per security's framing) | lucas42/lucos_claude_config#95 | Closed (completed) at 19:17:39Z (PR #99) |
| Coordinator closure flow: before closing any issue whose fix touched a production host, consult `monitoring.l42.eu/api/status` for failures on the affected host. Shipped as a STOP check in `references/triage-procedure.md` per [lucas42/lucos_claude_config@bad0c3e](https://github.com/lucas42/lucos_claude_config/commit/bad0c3e91aff06c8ebd5f99b35ff1eb69ab801b4) | lucas42/lucos_claude_config#96 | Closed (completed) at 18:21:49Z |
| Generalise the "Docker `Healthy` ≠ end-to-end working" pattern (two incidents in three weeks: 2026-05-09 lucos_creds CRLF, 2026-05-28 xwing network flush) into a cross-persona reference doc. Shipped as `references/healthcheck-depth.md` per `lucas42/lucos_claude_config#100`, propagated to sysadmin's verification template, code-reviewer's docker-compose review checklist, and SRE's diagnostic order. The mechanical-audit complement originally split out as `lucas42/lucos_repos#404` was subsequently closed as `not_planned` (see that row) — the reference doc in `lucas42/lucos_claude_config#100` is now the sole canonical home for the rule | lucas42/lucos_claude_config#97 | Closed (completed) at 19:23:44Z (PR #100) |
| ADR-0008: formalise the lucos deployment-model property "production hosts hold no source of truth for compose-managed state, so recovery from local Docker-state corruption must round-trip through CI." The procedural counterpart was the live-restore recovery procedure shipped under `#98` | lucas42/lucos/pull/199 | Merged at 18:47:18Z |
| Loopback-healthcheck audit rule (lucos_repos convention check): warn when a service's compose `healthcheck.test` is purely a loopback probe. Closed `not_planned` after architect identified that the proposal's load-bearing "Severity: Warning (not Fail)" mechanism doesn't actually exist in the convention engine, making the rule unimplementable as filed. The judgement-based version of the rule is now the sole canonical home, in `lucas42/lucos_claude_config#97`'s `healthcheck-depth.md` reference doc | lucas42/lucos_repos#404 | Closed (not planned) at 19:35:25Z |
| Sysadmin-reference update: document the "`docker stop` all containers before `sudo systemctl restart docker`" rule for recovering from live-restore network-init failure (the load-bearing finding from Stage 5 of this report). Shipped as the live-restore recovery procedure per `lucas42/lucos_claude_config#101` | lucas42/lucos_claude_config#98 | Closed (completed) at 19:27:15Z (PR #101) |

---

## Final state

At time of this report's final amendment (~2026-05-28 19:42 UTC, ~47 minutes after recovery):

- All six xwing-hosted services healthy on `monitoring.l42.eu/api/status` (`xwing`, `lucos_static_media`, `lucos_private`, `lucos_media_import`, `lucos_media_linuxplayer`, `lucos_docker_health`); the two cascading systems (`lucos_time`, depending on `staticmedia`) also healthy.
- `docker0` on xwing carrying the intended `fd42:dead:beef::1/64` ULA prefix; no public `/64` route on `docker0`. The original lucos#192 fix is in effect.
- Five-of-six follow-up issues closed `completed`, one closed `not_planned` (`lucas42/lucos_repos#404` — its proposed mechanism doesn't exist in the convention engine). All three associated PRs in `lucos_claude_config` (#99, #100, and #101 in that repo) merged. ADR-0008 (`lucas42/lucos#199`) merged.

One currently-unhealthy system on `monitoring.l42.eu/api/status` (`lucos_eolas` — `places-in-universe` check, "Swansea id=886 not reachable from Universe via `contained_in`") is **unrelated to this incident**: different host (not xwing), a content/data issue rather than infrastructure, and present before the xwing recovery completed. Noted here only to make the scope of this report's "all-clear" explicit: it's "all xwing-hosted systems healthy," not "estate-wide all-clear."
