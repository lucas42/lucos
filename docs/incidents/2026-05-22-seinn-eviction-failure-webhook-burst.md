...
If the accept path is structurally O(parse + push-to-array + log), and yet it's taking 5+ seconds to send a 202, then something is starving the Node event loop *between the accept handler and the response being written*. Plausible mechanisms, with the leading candidate identified during follow-up review:

1. **Synchronous `JSON.stringify(events)` on every webhook-status micro-transition (leading hypothesis).** `src/filesystem-state.js:15-17` does `fs.writeFile(STATE_FILE, JSON.stringify(events), …)` — note that the *stringify* is synchronous and runs on the main thread; only the file write itself is async. `save(events)` is invoked from `stateChange()` inside `src/routes/events.js`, which `webhooks.trigger()` then calls back into via every `summariseStatus()` pass — at minimum 7 times per 3-hook event (initial accept + per-hook pending + per-hook settled, plus retry-pending + retry-settled if any hook fails). During today's 6-minute burst this works out to ~490–910 main-thread serialisations of the entire events array. With the events array being newest-first and retained for 90 days, each `JSON.stringify` call is non-trivial CPU. This hypothesis fits the symptom shape exactly (event-loop starvation correlated with outbound burst, affecting both ingress 202 and `/_info` 200 alike) and was identified by `lucos-architect` during review of the post-merge investigation. A confirming probe is whether the `499` rate scales with `events.json` file size — `#485` is the place to settle it.
2. **Unbounded undici outbound fan-out + default pool semantics.** Node's built-in `fetch` (undici) defaults to 6 connections per origin in newer versions and 50 in older ones; behaviour under sustained burst depends on which default is in play and how settled-promise microtasks interact with the event loop. Many in-flight outbound fetches all settling at once produce a flood of microtasks; if microtask processing dominates the loop, even cheap handlers starve. This is the textbook event-loop-starvation pattern, but is more diffuse than (1) and may be amplifying it rather than being the primary cause.
3. **Express body-parser blocking.** The `router.use(express.json())` middleware at line 38 of `events.js` parses the request body before the handler runs. Large JSON payloads or contention on the parser would delay the 202. Probably not the dominant cause given event payloads are small, but cheap to rule out.
4. **GC pauses.** A process holding lots of in-flight webhook state, settled promises, and a 90-day event ring could be triggering long pauses. `--trace-gc` would settle it.
5. **TCP accept-queue saturation at the OS level.** With many concurrent inbound connections all hitting timeouts, the kernel's accept backlog could be filling faster than Node can `accept()` from it. Lowest-suspicion of the listed candidates.

None of these is *confirmed* by today's evidence — but (1) is far more specific than the rest and fits the symptom shape closely enough that the natural sequencing is "probe (1) first, fall back to the others if (1) is falsified". **This is the *real* root cause of today's 499s** (and, transitively, the 25 silent drops), and it needs its own SRE-side investigation ticket — tracked in Follow-up Actions. Until it lands, client-side retry would be treating a symptom rather than the cause, and could even compound the symptom (more retries → more concurrent fetches → more microtask load → more event-loop saturation → more retries needed).
...
|---|---|---|---|
| 1 | **Fix LRU desync race in service-worker cache eviction** — unified-lock fix proposed. The deeper root cause behind today's surface symptom, directly observed scrubbing LRU updates in production at ~15:25 UTC. | [`lucas42/lucos_media_seinn#472`](https://github.com/lucas42/lucos_media_seinn/issues/472) | Closed |
| 2 | **Investigate cache-eviction TypeError** (original framing). After `#472` shipped (2026-05-23), re-evaluation concluded: (a) corrupt-entry hypothesis falsified by SW-console probe; (b) TypeError was likely transient under extreme cache pressure; (c) `getCacheSizeWithMap` hardened to skip unreadable entries rather than aborting the whole pass. PR #479 closes this item. | [`lucas42/lucos_media_seinn#469`](https://github.com/lucas42/lucos_media_seinn/issues/469) | Closed |
| 3 | **Extend cache-thrash banner to fire on failed-eviction storms**, not only successful-eviction storms. Closes the in-page detection gap from this incident. | [`lucas42/lucos_media_seinn#470`](https://github.com/lucas42/lucos_media_seinn/issues/470) | Closed |
...
| 8 | **Investigate why loganne becomes unresponsive (returns 499s) under outbound webhook burst despite accept-202-fast.** The *real* root cause of today's 25 silent drops and the 86% `/_info` poll failures. Leading hypothesis (per architect's analysis on the ticket): synchronous `JSON.stringify(events)` in `filesystem-state.save()` is called from every `summariseStatus()` pass during webhook fan-out — ~490–910 main-thread serialisations of the entire events array during today's 6-minute burst. Fallback hypothesis space (undici microtask flood, body-parser blocking, GC pauses, TCP accept-queue backup) probed only if the leading is falsified. Must land before any client-side retry conversation. | [`lucas42/lucos_loganne#485`](https://github.com/lucas42/lucos_loganne/issues/485) | Open |
...
- **2026-05-23 (amended)**: `#472` (LRU desync fix) and `#470` (failure-detection gap) both shipped. Re-evaluation of `#469` (TypeError investigation): corrupt-entry hypothesis confirmed falsified; TypeError assessed as transient under extreme cache pressure (8 GB / 815 entries). `getCacheSizeWithMap` hardened in PR #479 to skip unreadable entries via try-catch rather than aborting the pass. Follow-up table updated: `#469` closed. Polling.js `:185`/`:221` logging bug (`url.method` → `action.method`) also fixed in the same PR.
