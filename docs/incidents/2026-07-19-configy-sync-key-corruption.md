...
| Fix the credential UI's CRLF-on-save corruption — the Round 1 write-path defect | `lucas42/lucos_creds#476` (commit `3ce4817`) | Done (closed `lucas42/lucos_creds#476`; superseded PR `lucas42/lucos_creds#477`) |
| Fix the regression in the shipped `lucas42/lucos_creds#476` fix (`3ce4817`): undefined POST `value` throws `TypeError`; reinstate the reviewed test coverage that never landed on `main` | `lucas42/lucos_creds#483` | Open |
