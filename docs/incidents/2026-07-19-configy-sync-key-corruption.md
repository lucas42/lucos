...
A routine rotation of the `lucos_creds_configy_sync` SSH keypair (`lucas42/lucos_creds#471`) exposed that the production `CONFIGY_SYNC_PRIVATE_SSH_KEY` had been stored corrupted, crash-looping the sync container for ~2.5 days and stalling config sync from `lucos_configy` into the credential store. The corruption happened twice, in two different ways: first CRLF line-endings (introduced when the key was saved through the credential UI, then truncated to its header by the deploy's line-based `.env` parser); then, during the first fix attempt, a stripped trailing newline (bash `$(...)` command substitution), producing a full-length-but-cryptographically-invalid key. The second round was insidious because the invalid key cleared the container's startup guard, so the container ran "healthy" and the monitoring dashboard went **all-green while sync was still completely broken**. It was resolved by producing and validating a known-good key in dev (root-caused by the sysadmin), copying the exact bytes into the prod store with a byte-safe write, and redeploying — verified end-to-end by a real sync completing, not by container health.
...
| 2026-07-19 10:11:04 | Deploy (pipeline 1085, `deploy-avalon`) recreates configy_sync; it re-reads the stored key — CRLF-corrupted, truncated to its 36-char header by the `.env` parser — and **crash-loops on the CR-guard**. Deploy job fails; `lucos_creds` circleci + `lucos_docker_health` avalon checks go red. |
...
| 2026-07-19 → 21 | Crash-loop continues (~1000+ restarts). The `configy_sync` scheduled-job check reads **green throughout** despite no successful sync in >2.5 days — the job stopped reporting, so the check failed open (see `lucas42/lucos_schedule_tracker#96`). |
...
### Delayed / masked detection — the false-green dashboard

Two independent monitoring blind spots meant the dashboard could not be trusted through either round:

1. **Fail-open scheduled-job checks.** The `configy_sync` check promises "the most recent run happened in the last 10800s", yet read green over a >2.5-day dead job. schedule-tracker holds no registry of *expected* jobs, so a job that stops reporting entirely is *absent*, not *stale*, and absence reads green. A check that can only go red on *reported* failures is blind to a silently-dead job.
