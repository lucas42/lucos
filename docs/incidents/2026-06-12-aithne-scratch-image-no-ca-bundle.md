...
| Decide whether new Go services should default to `distroless/static` instead of `scratch` (convention default) | `lucas42/lucos#242` | Open — parked at lucas42's "not now" decision (Ideation/Low, revisitable); `scratch` remains the default, `#240` covers the gap |
