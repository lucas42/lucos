...
The honest trade-off: `scratch` has a genuine merit — absolute-minimum attack surface, nothing in the image to exploit — so this is not a blanket "scratch is wrong." But for any service that makes (or might ever make) outbound HTTPS, `distroless/static` is the better default, and aithne is the proof: "makes no outbound calls" was true at creation and silently became false later. This separates into two follow-ups: `lucas42/lucos#240` patches the symptom (audit existing scratch images, encode the CA-bundle convention) and is the agreed action, while `lucas42/lucos#242` carried the class-kill question — "should new Go services default to `distroless/static` unless they provably need nothing outbound?". They were deliberately split so an open-ended decision wouldn't gate the ready audit work — which is itself the incident's lesson (a tracked issue, not a deferred sub-bullet) applied. **lucas42 decided not to adopt the new default** — `#242` is closed (`not_planned`, revisitable if the trade-off is ever worth reopening), `scratch` remains the default, and the `#240` audit + CA-bundle convention cover the immediate gap. (Reframe per lucos-architect's review; default decision per lucas42.)
...
| Decide whether new Go services should default to `distroless/static` instead of `scratch` (convention default) | `lucas42/lucos#242` | Closed (`not_planned`) — distroless default not pursued at this stage; `scratch` remains the default, `#240` covers the gap. Revisitable |
