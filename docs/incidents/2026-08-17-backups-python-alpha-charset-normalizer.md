...
`lucos-security`'s framing of this is stronger than the reproducibility one and is worth stating in its own right: because the build re-resolves on every commit, there is **no code-review gate at all** between a package publishing on PyPI and that code executing on a host holding production SSH credentials. Dependabot exists to provide precisely that gate, and it is being silently routed around. This is not specific to `lucos_backups` — **all 8 live Python repos** install without a lockfile-enforcing flag (lucas42/lucos_repos#488). Of those, 3 are re-resolving today; the other 5 agree with their lockfile only by coincidence of which unpinned pipenv version the build happens to install, with nothing asserting that agreement and no signal when it stops holding. Correct-by-coincidence and silently reversible is a stronger argument for a standing convention than "all of them are broken" would have been.
...
| **Crash-loop detection:** `lucos_docker_health` reported 42 recoveries during an outage that never recovered (Stage 5) | lucas42/lucos_docker_health#108 | **Done** — lucas42/lucos_docker_health#109, merged 00:10Z 08-18, flagging crash-looping containers via rising `RestartCount` |
...
The Stage 5 crash-loop detection gap was raised as lucas42/lucos_docker_health#108 and is **already fixed** — lucas42/lucos_docker_health#109, merged 00:10Z on 2026-08-18, flags crash-looping containers via a **rising `RestartCount`** rather than sampling health status. That works because `RestartCount` is monotonic, so comparing it between polls is phase-independent: it turns an oscillating sample into a level signal, and needs one map in a loop that already polls every 60 seconds. (Requiring a rise across two consecutive polls keeps an ordinary deploy restart from alerting.) The second lever raised there — whether the "any of the last N runs" window is a config value that could simply be set to 1 for this job — remains worth checking, and would need no code at all.
