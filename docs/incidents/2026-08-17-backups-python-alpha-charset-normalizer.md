...
| **Crash-loop detection:** `lucos_docker_health` reported 42 recoveries during an outage that never recovered (Stage 5) | lucas42/lucos_docker_health#108 | Open |
...
The Stage 5 crash-loop detection gap is tracked as lucas42/lucos_docker_health#108, which proposes two levers: treat a **rising `RestartCount`** as unhealthy independent of health status (it is monotonic, so comparing it between polls is phase-independent — it turns an oscillating sample into a level signal, and needs one map in a loop that already polls every 60 seconds, requiring a rise across two consecutive polls so an ordinary deploy restart doesn't alert); and, to check first because it may need no code at all, whether the "any of the last N runs" window is a config value that can simply be set to 1 for this job.
