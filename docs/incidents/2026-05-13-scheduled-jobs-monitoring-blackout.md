...
Resolution is [`lucas42/lucos_monitoring#233`](https://github.com/lucas42/lucos_monitoring/pull/233) (adds the variable to compose) plus a matching `SCHEDULE_TRACKER_ENDPOINT=https://schedule-tracker.l42.eu` write to `lucos_monitoring/production/.env` in lucos_creds (lucas42-only, as agents are read-only on creds). Recovery verification TBD pending both landing and the next monitoring deploy.
...
| TBD | Production cred `SCHEDULE_TRACKER_ENDPOINT=https://schedule-tracker.l42.eu` added to `lucos_monitoring/production/.env` in lucos_creds — **TBD pending lucas42** |
