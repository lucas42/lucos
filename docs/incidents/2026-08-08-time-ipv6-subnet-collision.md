...
**The shape these share.** Three of the errors above are the same error: **I asserted something I had not executed** — a duration, a persistence claim, and a recovery command (the rollback recipe, in stage 2). The probe-discipline habit in `agents/sre-ops-checks.md` covers *observations* well and did real work today. It says nothing about **claims of the form "and if X fails, do Y"**, which are predictions about a path nobody has walked.

Predictions about procedures are the higher-stakes half, and `lucos-architect` put the reason better than I did: a bad *value* fails immediately, in front of the person typing it, whereas a bad *recovery step* fails only in the state it was written for — mid-incident, after the destructive action, with no other route back. The rollback recipe was caught only because they refused to accept an unrehearsed recovery path and `lucos-system-administrator` executed it on a disposable project.

Worth recording precisely, because the tempting lesson is the wrong one: **the hypothesis about *why* it would fail was also wrong.** The worry was that a container would not reattach to a network with a new ID; Compose never got that far, because it would not adopt the network at all. Reasoning harder does not find these. Rehearsing does, and it works precisely because you cannot know in advance which assumption is the broken one.

The generalisable form, also `lucos-architect`'s: **enumerate what the *recovery* depends on, not just what you are changing.** The pre-flight here proved the network could be created and said nothing about whether CircleCI could deploy or primary DNS was serving — both of which the recovery needs. Same shape as the subnet probe, one level up.
