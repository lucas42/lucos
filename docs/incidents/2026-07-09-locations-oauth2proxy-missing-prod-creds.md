...
`/_info` is served by a **separate local health-aggregator** — `info_server.py`, a small Python process on port 8080 running inside the `otfrontend` container itself — at an nginx location (`location /_info`) that carries **no `auth_request` directive** (`auth_basic off; proxy_pass http://127.0.0.1:8080/_info;`). It never touches oauth2-proxy or otrecorder's own health: it synthesises its own checks (mosquitto TLS expiry, location-freshness via a fast local call to otrecorder's `/api/0/last`) and returns 200 with the results in the body, per the standard lucos `/_info` convention. So monitoring stayed green (54/54) **not** because "the app behind the sidecar was healthy," but because this endpoint is deliberately built never to depend on the auth chain in front of it.

This is **not** a universal property of sidecar-fronted services — the distinction matters for future readers. Plenty of lucos services proxy `/_info` straight through to their backend app and *would* be blinded by a failing front proxy. This one isn't, by design, which is the safer arrangement. (The gated backend, otrecorder, is the third-party `owntracks/recorder` image — not a lucos-authored app.)
