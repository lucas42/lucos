...
Approximately 10 minutes after the first arachne outage was resolved (via PR #278), a second outage began. PR #261 (*Cache 3rd party ontology files locally*), part of the same batch deploy, had renamed `systems_to_graphs` to `live_systems` in `ingestor/triplestore.py`. The `ingestor/server.py` file — which handles incoming Loganne webhooks — was not updated to use the new name. The ingestor crash-looped on startup with an `ImportError`, which (via `depends_on: condition: service_healthy`) blocked the `web` container from starting.
...
| ~15:05 | Ingestor container replaced with image from PR #261; starts crash-looping: `ImportError: cannot import name 'systems_to_graphs'` |
...
| ~15:12 | Root cause identified: PR #261 renamed `systems_to_graphs` → `live_systems` in `triplestore.py`; `server.py` not updated |
...
Both import from `triplestore.py`, but PR #261 only updated `ingest.py`. `server.py` is a less frequently touched file and was overlooked.
