...
The `KEY_LUCOS_EOLAS` Bearer token was leaked to `id.loc.gov` in HTTP `Authorization` headers an estimated 42+ times during the burst (one per event arachne attempted to fetch from the misrouted URL). The actual token value is not included in this report; the credential has been rotated by lucas42 via lucos_creds. The leak path was addressed in lucas42/lucos_arachne#444 (merged 23:51 UTC) by restricting auth headers to trusted lucos hosts only. Both the credential rotation and the structural fix are in place.
