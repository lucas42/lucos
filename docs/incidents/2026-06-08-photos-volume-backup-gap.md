...
| 2026-06-12 23:45 | lucos_backups **v1.1.13** deployed to avalon (incremental path live for photos_photos, with lucas42/lucos_configy#230's `backup_strategy` field). |
...
**Latent risk for other volumes (raised by lucos-system-administrator post-merge):** the 600 s timeout is not photos-specific — it bounds the full-snapshot `scp` for *every* volume still on that strategy, so any volume that grows past the cliff would hit the identical silent-daily-failure mode. Current headroom is comfortable, verified 2026-06-14: the only volumes at/over the cliff are `lucos_photos_photos` (now `incremental`) and `lucos_docker_mirror_cache` (41 GB but `skip_backup: true` — a regenerable pull-through cache), and the largest *backed-up* full-snapshot volume is `lucos_media_metadata_api_db` at ~239 MB — ~28× under the threshold. The incremental `rsync --link-dest` path is now the proven mitigation: opt a volume in (per ADR-0002) when it approaches the cliff, rather than bumping the timeout. No standing detection was added — a per-volume "duration approaching timeout" check carries per-volume config and false-positive triage, whereas the failure is already caught within a day by `create-backups` going red and is recoverable via the incremental migration above, so the effort isn't justified. Risk accepted with the headroom documented here; no migration ticket or monitoring-check follow-up filed.
...
| Build a CI/staging harness exercising the incremental rsync-over-ProxyJump path before deploy | — | **Not filed — risk accepted.** The path needs the real gateway + aurora reachable and would be non-deterministic in CI (high build/maintain cost); the failure mode is an internal backup delayed by ≤1 day, and the existing `create-backups` monitoring check already catches both a hard failure and the data-stranding case (the run reports failed → check goes red) within a day — and that cannot false-green, because the publish exit propagates as `UnexpectedExit` (see Analysis). The effort doesn't justify the marginal detection gain. Captured here rather than as a low-value ticket. |
| GC orphaned `<date>.partial/` directories on the incremental target (unbounded count on the capped aurora NAS) — gap exposed (not triggered) by this incident, raised by lucos-architect | lucas42/lucos_backups#333 | Open (P3) |
